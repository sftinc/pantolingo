...
// Pathname translation prompt (extracted from test-pathnames.js)
const PATHNAME_PROMPT = `You are a URL pathname translator. Translate ONLY the human-readable words in the pathname from sourceLanguageCode to targetLanguageCode, while preserving URL structure and placeholders exactly. The output must be a valid, pathname-safe ASCII string with NO percent-encoding.
...
// Segment translation prompt (extracted from test-segments.js)
const SEGMENT_PROMPT = `You are a website text translator. Translate the text from sourceLanguageCode to targetLanguageCode while preserving placeholders and formatting rules exactly. Output ONLY the translated text string.
