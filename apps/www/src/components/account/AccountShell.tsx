...
const COLOR_CLASSES: Record<WebsiteColor, { btn: string; hover: string; avatar: string; chevron: string; text: string }> = {
...
export function AccountShell({ currentWebsite, websites, userName, userProfile, signOutAction, children }: AccountShellProps) {
	...
	const currentColorKey = (currentWebsite.uiColor as WebsiteColor) || getWebsiteColor(currentWebsite.name)
								...
								{otherWebsites.length > 0 && (
									<>
										...
										{otherWebsites.map((site) => {
											const siteColorKey = (site.uiColor as WebsiteColor) || getWebsiteColor(site.name)
