...
/**
 * Shared verification page component for /signup/verify and /login/verify
 * Displays Turnstile widget and auto-submits on successful verification
 */
export function VerifyHumanForm({ email, turnstileSiteKey, backHref, backText }: VerifyHumanFormProps) {
	...
	const handleVerify = (token: string) => {
		...
		// Create form data with token and flow, then submit
		const formData = new FormData()
		formData.set('turnstileToken', token)
		formData.set('flow', backHref === '/signup' ? 'signup' : 'login')
		formAction(formData)
	}
		...
		<main className="flex min-h-screen flex-col">
			...
			<div className="flex flex-1 flex-col items-center justify-center p-6">
				<div className="w-full max-w-md bg-[var(--card-bg)] p-10 rounded-lg shadow-[0_2px_8px_var(--shadow-color)]">
					...
					<p className="text-base text-[var(--text-muted)] mb-6 text-center">{email}</p>
